...
def alternate_gradient(N_iter, initial_point, equilibrium_point, params, func_mat, tol=1.0e-5):
    x0, y0 = initial_point
    x, y = x0, y0
    x_nash, y_nash = equilibrium_point
    L_xx, L_yy, L_xy, L_yx, mu_x, mu_y = params

    LR_x = 0.5*(np.sqrt(mu_y) / (L_xy*np.sqrt(L_xx)))
    LR_y = 0.5*(np.sqrt(mu_x) / (L_xy*np.sqrt(L_yy)))
    ...
    for i in range(N_iter):
        g_x, _ = grad(func_mat, x, y)

        x = x - LR_x*g_x
        
        _, g_y = grad(func_mat, x, y)
        y = y + LR_y*g_y
